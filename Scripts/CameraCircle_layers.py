...
class NSOT_Props(bpy.types.PropertyGroup):
    ...
    # Render progress (best-effort; updates between frames)
    ...
    def execute(self, context):
        ...
        try:
            for i, cam in enumerate(cams, start=1):
                ...
                # Update progress before each frame render (UI updates between frames, not during a frame)
                p.progress_current = i
                wm.progress_update(i)

                # Nudge UI redraw so the panel label updates between frames
