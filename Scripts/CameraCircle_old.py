bl_info = {
    "name": "Nerfstudio Object-Only Splat Exporter (Spherical)",
    "author": "ChatGPT",
    "version": (1, 2, 0),
    "blender": (3, 6, 0),
    "location": "View3D > Sidebar (N) > Nerfstudio",
    "description": "Create spherical camera rig (spacing-driven, includes top+bottom), render RGBA PNGs, write alpha-derived masks, export Nerfstudio transforms.json with mask_path",
...
def clear_prefixed_cameras(coll, prefix):
...
def mat_to_list4x4(m: Matrix):
    return [[float(m[r][c]) for c in range(4)] for r in range(4)]


def camera_angle_x(lens_mm: float, sensor_w: float) -> float:
    return 2.0 * math.atan((sensor_w * 0.5) / lens_mm)


def make_camera(name, mw: Matrix, lens_mm, sensor_w, clip_start, clip_end):
...
def set_render_settings(scene, engine, res_x, res_y, png_compression, film_transparent, cycles_samples, cycles_denoise):
...
def alpha_png_to_mask(png_path, mask_path, threshold):
    try:
        from PIL import Image
    except Exception as e:
        raise RuntimeError(
            "Pillow (PIL) is missing inside Blender's Python. Install it into Blender Python, not system Python."
        ) from e

    im = Image.open(png_path).convert("RGBA")
    a = im.split()[-1]
    m = a.point(lambda v: 255 if v > threshold else 0).convert("L")
    m.save(mask_path)


def spherical_points_by_spacing(max_dist, sphere_radius):
...
class NSOT_Props(bpy.types.PropertyGroup):
    ...
    alpha_threshold: bpy.props.IntProperty(name="Alpha Threshold", default=1, min=0, max=255)
...
class NSOT_OT_create_cameras(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        pts = spherical_points_by_spacing(p.max_dist, p.radius)
        ...
        for local_pos, tag in pts:
            ...
            if tag == "BOTTOM":
                name = f"{p.name_prefix}{idx:03d}_BOTTOM"
            elif tag == "TOP":
                name = f"{p.name_prefix}{idx:03d}_TOP"
            else:
                name = f"{p.name_prefix}{idx:03d}"
        ...
        self.report({"INFO"}, f"Created {len(cams)} cameras in '{p.collection_name}'")
        return {"FINISHED"}


class NSOT_OT_export_dataset(bpy.types.Operator):
    bl_idname = "nsot.export_dataset"
    bl_label = "Render + Export (Images, Masks, transforms.json)"
    bl_options = {"REGISTER"}
    ...
    def execute(self, context):
        ...
        for cam in cams:
            ...
            msk_rel = f"masks/{filename}"

            img_abs = os.path.join(out_images, filename)
            msk_abs = os.path.join(out_masks, filename)
            ...
            try:
                alpha_png_to_mask(img_abs, msk_abs, p.alpha_threshold)
            except Exception as e:
            ...
            frames.append({
                "file_path": img_rel,
                "mask_path": msk_rel,
                "transform_matrix": mat_to_list4x4(cam.matrix_world),
            })

        tf = {
            "camera_angle_x": camera_angle_x(p.focal_mm, p.sensor_width_mm),
            "frames": frames,
        }

        tf_path = os.path.join(out_root, "transforms.json")
        with open(tf_path, "w", encoding="utf-8") as f:
            json.dump(tf, f, indent=2)

        self.report({"INFO"}, f"Exported {len(frames)} frames to {out_root}")
        return {"FINISHED"}
